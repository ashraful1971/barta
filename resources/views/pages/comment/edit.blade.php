        ...
        <article class="bg-white border-2 border-black rounded-lg shadow mx-auto max-w-none px-4 py-5 sm:px-6">
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div class="flex-shrink-0">
                            <img class="h-10 w-10 rounded-full object-cover"
                                src="{{ $comment->post->user->avatar ? asset('storage/' . $comment->post->user->avatar) : asset('images/avatar.png') }}"
                                alt="Tony Stark">
                        </div>
                        <!-- /User Avatar -->
            ...
            <div class="py-4 text-gray-700 font-normal space-y-2">
                @if ($comment->post->image)
                    <img src="{{ asset('storage/' . $comment->post->image) }}" class="min-h-auto w-full rounded-lg object-contain"
                        alt="Image">
                @endif
                <p>
            ...
            <div class="flex items-center gap-2 text-gray-500 text-xs my-2">
                <span class="">{{ $comment->post->created_at->diffForHumans() }}</span>
                <span class="">•</span>
                <span>{{ $comment->post->comments->count() }} {{ $comment->post->comments->count() > 1 ? 'comments' : 'comment' }}</span>
                <span class="">•</span>
                <span>450 views</span>
            </div>
                ...
                <div>
                    <div class="flex items-start space-x-3">
                        ...
                        <div class="text-gray-700 font-normal w-full">
                            <input name="post_id" type="hidden" value="{{ $comment->post->id }}">
                            <textarea name="content" x-data="{
                                ...
                                style="height: 38px;">{{ $comment->content }}</textarea>
                        </div>
                    </div>
                </div>
